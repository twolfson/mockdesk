...
function Application(container, config) {
  ...
  var elementPaletteEl = D.DIV({
    ...
    return Object.keys(elementMap).map(function createElement (elementName) {
      ...
      unidragger.render = function () {
        // If there is no dragX/dragY, then do nothing
        if (this.dragX === undefined) {
          return;
        }

        // Otherwise, update our element position
        console.log('animate');
        element.setStyle({
          left: this.dragX + 'px',
          position: 'absolute',
      ...
      unidragger.dragMove = function handleDragMove (evt, pointer, moveVector) {
        // Save the drag updates as they occur
        this.dragX = this.dragStartPoint.x + moveVector.x;
        this.dragY = this.dragStartPoint.y + moveVector.y;
      };
