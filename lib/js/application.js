...
function Application(container, config) {
  ...
  workspaceEl.addEventListener('click', function handleClick (evt) {
    ...
    for (; i < len; i++) {
      ...
      var widget = this.layers[i];
      var widgetBounds = widget.getBounds();
      if (widgetBounds.left <= evt.pageX && widgetBounds.right >= evt.pageX &&
          widgetBounds.top <= evt.pageY && widgetBounds.bottom >= evt.pageY) {
        matchingBounds = widgetBounds;
      }
    }

    // If we had matching bounds, then move our handle element to that location
    // TODO: Should we have something like `selectedWidget`/`selectedLayer`?
    if (matchingBounds) {
