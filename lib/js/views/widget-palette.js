...
WidgetPalette.prototype = {
  ...
  bind: function (container) {
    ...
    // Add drag bindings
    // DEV: If all of these event listeners get unwieldy, consider event delegation for clicking/dragging
    ...
    [].forEach.call(widgetContainers, function bindDrag (widgetContainer) {
      ...
      draggie._dragPointerDown = function (event, pointer) {
        ...
        return Draggabilly.prototype._dragPointerDown.call(this, event, pointer);
      };
      draggie.resetWidget = function () {
        // Reset the original widget to its original position
        // DEV: This is applied for both static clicks and drags
      ...
      draggie.on('dragEnd', function handleDragEnd (evt, pointer) {
        ...
        var workspaceBounds = that.app.getWorkspaceBounds();
        ...
        if (true) {
          ...
          var workspaceScrollInfo = that.app.getWorkspaceScrollInfo();
          var newWidget = new WidgetClass({
            width: 100,
            height: 100,
            style: {
              left: widgetBounds.left - workspaceBounds.left + workspaceScrollInfo.left,
              position: 'absolute',
              top: widgetBounds.top - workspaceBounds.top + workspaceScrollInfo.top
            }
          });
  ...
  unbind: function () {
    ...
    delete this.unbindFns;
  },
  destroy: function () {
    // Unbind from our container
    this.unbind();
