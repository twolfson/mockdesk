...
exports.init = function (mockdesk) {
  ...
  function createMenuFromTemplate(menuParams) {
    ...
    var menuItems = menuParams.map(function createMenuItem (menuItemParams) {
      ...
      // TODO: Too tired to continue on for now... I didn't expect this to run on so long...
      //   Solution: Create a function that is another parameter next to `menuParams` to resolve the parent menu
      //   Then, look up the menu item by its index
      //   Then we reference the `buildFromTemplate` content
    ...
    var menu = new Menu();
    menuItems.forEach(function appendItem (menuItem, i) {
      var item = menuParams[i];
      for (var key in item) {
        var value = item[key];
