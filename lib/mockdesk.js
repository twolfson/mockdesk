...
var mockdesk = {
  ...
  toggleLiveReloadViaMenuItem: function (menuItem) {
    // DEV: We toggle all windows since Electron doesn't seem to support updating `checked` property
    //   outside of `on('change')`. We would toggle it via an IPC listener in `app-menu` (emitter in `browser.js`)
    logger.debug('Toggling LiveReload in all browser windows');
    console.log('wtf', arguments);
    menuItem.checked = !menuItem.checked;
    console.log('wtf222', menuItem.checked);
    BrowserWindow.getAllWindows().forEach(function sendToggleLiveReload (browserWindow) {
      browserWindow.send('toggle:livereload');
    });
  }
};
